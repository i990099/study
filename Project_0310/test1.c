...
unsigned char* crcVal(unsigned char* pcMess, unsigned int wLen)
{
	unsigned int Index; 
	...
	nCRCData = nCRCData >> 8 | nCRCData << 8;
	unsigned char cc[2] = { 0 };
	cc[0] = nCRCData & 0xff;
	cc[1] = ((nCRCData >> 8) & 0xff);
	//printf("%x  %x\n", nCRCData >> 8, ((nCRCData >> 8) & 0xff));
	printf("%x  %x\n",cc[0],cc[1]);
	//char* pcc = cc;
	pcMess = cc;
	return pcMess;
	//return nCRCData & 0xffff;
...
int main() {
	...
	unsigned char* pcc = crcVal(pc, len);
